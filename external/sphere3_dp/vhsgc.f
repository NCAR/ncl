...
c     if(lvhsgc .lt. 2*(lzz1+labc)+nlon+15) return
     ...
     +    (2*NT*NLON+MAX0(6*NLAT,NLON))) RETURN
      ...
      JW1 = LWZVIN + 1
      JW2 = JW1 + LWZVIN
      CALL DVHSGC1(NLAT,NLON,ITYP,NT,IMID,IDVW,JDVW,V,W,MDAB,NDAB,BR,BI,
     +            CR,CI,IDV,WORK,WORK(IW1),WORK(IW2),WORK(IW3),
     +            WORK(IW4),WORK(IW5),WVHSGC,WVHSGC(JW1),WVHSGC(JW2))
      RETURN
      END
      SUBROUTINE DVHSGC1(NLAT,NLON,ITYP,NT,IMID,IDVW,JDVW,V,W,MDAB,
