...
c
      SUBROUTINE DVISEQU(NLAT,NLON,H,LEN,EYER,EYELAT,EYELON,WK,LWK,IWK,
     +                  LIWK,IERROR)
      DOUBLE PRECISION H
...
c     ****     mid-cell interpolation, calculation of polar values
      CALL DINTERP(H,LEN,M,N,WK(NW1),WK(NW2),IWK(NIFLAG))
c     ****     transform grid points to cartesian coordinates
      CALL DSPTC(H,LEN,M,N,WK(NCLAT),WK(NSLAT),WK(NX),WK(NY),WK(NZ))
...
c     ****     perform triangulation
      CALL DTRIANG(M,N,WK(NX),WK(NY),WK(NZ),ITRI,WK(NX1),WK(NY1),
...
c     ****     call surface plotting routine
     ...
     +           WK(NWRK),IWK(NIFLAG))
      RETURN
      END
      SUBROUTINE DINTERP(H,LEN,M,N,W1,W2,IFLAG)
      DOUBLE PRECISION H
      DOUBLE PRECISION W1
      DOUBLE PRECISION W2
...
c     ****     iflag(j,i)=16 if diagonal is (j+1,i), (j,i+1)
    ...
    9 CONTINUE
      RETURN
      END
      SUBROUTINE DSPTC(R,LEN,M,N,CLAT,SLAT,X,Y,Z)
      DOUBLE PRECISION R
...
c
      RETURN
      END
      SUBROUTINE DTRIANG(M,N,X,Y,Z,ITRI,X1,Y1,Z1,X2,Y2,Z2,X3,Y3,Z3,
     +                  ITYP,IFLAG,MST)
      DOUBLE PRECISION X
      DOUBLE PRECISION Y
      DOUBLE PRECISION Z
...
c
      CALL DVSURF1(XEYE,YEYE,ZEYE,NTRI,X1,Y1,Z1,X2,Y2,Z2,X3,Y3,Z3,ITYPE,
     +            WORK,WORK(NTRI+1),WORK(2*NTRI+1),WORK(3*NTRI+1),
     +            WORK(4*NTRI+1),WORK(5*NTRI+1),WORK(6*NTRI+1),
     +            WORK(7*NTRI+1),WORK(8*NTRI+1),WORK(9*NTRI+1),
     +            WORK(10*NTRI+1),WORK(11*NTRI+1),WORK(12*NTRI+1),
     +            WORK(13*NTRI+1),IWORK,IWORK(NTRI+1),IWORK(2*NTRI+1),
     +            IWORK(4*NTRI+1))
      RETURN
      END
...
c
   ...
   87 CONTINUE
      PMIN = 1.1D0*PMIN
      PMAX = 1.1D0*PMAX
      CALL DSET(0.D0,1.D0,0.D0,1.D0,PMIN,PMAX,PMIN,PMAX,1)
...
c
  ...
  171                     CALL DLINE(PX4,PY4,PX5,PY5)
                          GO TO 460
c
...
c     ****     to line joining origin and eye
      ...
      DO 100 I = 1,M
          DO 100 J = 1,N
              CALL DPRJCT(1,XEYE,YEYE,ZEYE,X(J,I),Y(J,I),Z(J,I),PX(J,I),
     +                   PY(J,I))
  100 CONTINUE
      RETURN
      END
