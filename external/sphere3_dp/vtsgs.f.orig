...
c
      ...
      lwts = ltheta
      jw1 = lgaqd+1
      CALL VETG1(NLAT,NLON,IMID,WVTS,WVTS(LZIMN+1),DWORK,WORK,
     1              WORK(IW1),DWORK(IW2),DWORK(JW1),IERROR)
      if(ierror .ne. 0) return
      call hrffti(nlon,wvts(2*lzimn+1))
      return
      end
      subroutine vetg1(nlat,nlon,imid,vb,wb,dwork,vin,wvbin,
     1                       theta,wts,ierror)
      dimension vb(imid,*),wb(imid,*),vin(imid,nlat,3),wvbin(*)
      double precision dwork(*),theta(*),wts(*)
      mmax = min0(nlat,(nlon+1)/2)
c     lwork = 2*nlat*(nlat+2)
      lwork = nlat*(nlat+2)
