...
c **********************************************************************
c
      SUBROUTINE DSFVPES(NLAT,NLON,ISYM,NT,SF,VP,IDV,JDV,BR,BI,CR,CI,
     +                   MDB,NDB,WSHSES,LSHSES,WORK,LWORK,IERROR)
      ...
      DOUBLE PRECISION WSHSES(LSHSES),WORK(LWORK)
...
c
      ...
      IWK = IS + NLAT
      LWK = LWORK - 2*MN - NLAT
      CALL DSFVPES1(NLAT,NLON,ISYM,NT,SF,VP,IDV,JDV,BR,BI,CR,CI,MDB,NDB,
     +             WORK(IA),WORK(IB),MAB,WORK(IS),WSHSES,LSHSES,
     +             WORK(IWK),LWK,IERROR)
      RETURN
      END

      SUBROUTINE DSFVPES1(NLAT,NLON,ISYM,NT,SF,VP,IDV,JDV,BR,BI,CR,CI,
     +                   MDB,NDB,A,B,MAB,FNN,WSHSES,LSHSES,WK,LWK,
     +                   IERROR)
      IMPLICIT NONE
     ...
     +                 CI(MDB,NDB,NT)
      ...
      DOUBLE PRECISION WSHSES(LSHSES),WK(LWK),FNN(NLAT)
      INTEGER N,M,MMAX,K
c
...
c
      CALL DSHSES(NLAT,NLON,ISYM,NT,SF,IDV,JDV,A,B,MAB,NLAT,WSHSES,
...
c
      CALL DSHSES(NLAT,NLON,ISYM,NT,VP,IDV,JDV,A,B,MAB,NLAT,WSHSES,
     +           LSHSES,WK,LWK,IERROR)
      RETURN
      END
