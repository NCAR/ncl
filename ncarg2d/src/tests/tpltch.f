C
C	$Id: tpltch.f,v 1.1.1.1 1992-04-17 22:33:30 ncargd Exp $
C
...
C                        the package PLOTCHAR and to test them.
C
...
C
C                        In addition, several frames are produced.  To
...
C LANGUAGE               FORTRAN
C
C ALGORITHM              TPLTCH first produces four plots showing the
C                        complete complex character set and another
C                        four showing the complete duplex character
C                        set.  Each plot shows a rectangular grid of
C                        characters.  The character in a given column
C                        and row is that character which would be
C                        written by PLCHHQ using the function code
C                        shown at the top of the column with the
C                        standard FORTRAN character shown at the
C                        beginning of the row.  (Alternatively, each
C                        character could be selected by using as a
C                        function code the octal number obtained by
C                        summing the four-digit octal number for the
C                        column and the two-digit octal number for the
C                        row.)  By default, these eight plots are put
C                        on two frames.  One can change the value of
C                        the variable ICMP, in a data statement, from
C                        a 1 to a 0 to cause each of the eight plots
C                        to be put on a separate frame.
C
C                        Next, TPLTCH produces a frame showing various
C                        capabilities of PLCHHQ.
C
C                        Finally, TPLTCH produces a frame showing the
C                        entire medium-quality character set for five
C                        different values of the height-to-width ratio.
C
C                        As of June of 1990, two additional frames have
C                        been added, one showing all the new characters
C                        available from fontcap databases and the other
C                        showing new capabilities added to the package.
C
C ---------------------------------------------------------------------
C
