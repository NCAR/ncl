...
C
  ...
  710     FORMAT(' Local GKS error number',I5,
     +         ' issued from the X driver:')       
        ...
        DO 720 I=160,1,-1
          L2(I) = CHAR(XERMSG(I))
          IF (IFS .EQ. 0) THEN
            IF (XERMSG(I).NE.ICHAR(' ') .AND. XERMSG(I).NE.0) THEN
              NUMC = I
              IFS = 1
            ENDIF
          ENDIF
  720   CONTINUE
C
        WRITE(ERRFIL,740) (L2(LL),LL=1,NUMC)
  740   FORMAT(160A1)
        RETURN
      ELSE IF (ERRNR .EQ. -109) THEN
C
...
C
  ...
  700   FORMAT(' GKS ERROR NUMBER',I4,' ISSUED FROM SUBROUTINE ',A6,':')
      ENDIF
C
C  Write out the error message to the GKS error file.
C
      IF (ERRNR.EQ.1) THEN
        WRITE (ERRFIL,  1) L1
      ...
      ELSE IF (ERRNR.EQ.-109) THEN
        IF (CUFLAG.EQ.-1) WRITE(ERRFIL, 932) L1
      ELSE IF (ERRNR.EQ.-110) THEN
        WRITE(ERRFIL, 933) L1
      ELSE IF (ERRNR.EQ.-111) THEN
        WRITE(ERRFIL, 934) L1
      ELSE IF (ERRNR.EQ.-112) THEN
        WRITE(ERRFIL, 935) L1
      ELSE
        WRITE(ERRFIL, 921) L1
      ENDIF
C
   ...
   27 FORMAT(A52/' --WORKSTATION INDEPENDENT SEGMENT STORAGE IS NOT OPEN
     +')
   28 FORMAT(A52/' --WORKSTATION INDEPENDENT SEGMENT STORAGE IS ALREADY
     +OPEN')
   ...
   36 FORMAT(A52/' --SPECIFIED WORKSTATION IS WORKSTATION INDEPENDENT SE
     +GMENT STORAGE')
   38 FORMAT(A52/' --SPECIFIED WORKSTATION IS NEITHER OF CATEGORY INPUT
   ...
   84 FORMAT(A52/' --STYLE (PATTERN OR HATCH) INDEX IS LESS THAN OR EQUA
     +L TO ZERO')
   85 FORMAT(A52/' --SPECIFIED PATTERN INDEX IS INVALID')
 ...
 2003 FORMAT(A52/' --INVALID DATA RECORD')
  ...
  926 FORMAT(A52/' --THE NCAR GKS IMPLEMENTATION REQUIRES THAT WISS BE A
     +CTIVE BEFORE A SEGMENT IS CREATED')
  ...
  931 FORMAT(A52/' --ILLEGAL PLACEMENT OF CALL TO ASSIGN PICTURE NAME:',
     +          /'     ',A60)
  932 FORMAT(A52/' --THIS FUNCTION NOT YET IMPLEMENTED FOR WORKSTATIONS
     +OF CATEGORY OUTPUT')
  933 FORMAT(A52/' --CHARACTER STRING TOO LARGE')
  934 FORMAT(A52/' --SYSTEM ERROR')
  935 FORMAT(A52/' --ONLY ONE METAFILE WORKSTATION CAN BE OPEN AT A TIME
     +')
C
      RETURN
      END
