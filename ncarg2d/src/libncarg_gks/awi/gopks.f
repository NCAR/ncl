C
C	$Id: gopks.f,v 1.1.1.1 1992-04-17 22:33:40 ncargd Exp $
C
...
C     CHECK IF GKS IS IN PROPER STATE
      ...
      IF (IER .NE. 0) RETURN
C
C     INITIALIZE ERROR STATE LIST
C
      CALL GZINES
C
C     SPECIFY ERROR FILE IN THE GKS ERROR STATE LIST
C
      ERF = ERRFIL
C
...
C
      CALL GZINSL
C
C     SET GKS OPERATING STATE TO GKSOP
C
      OPS = GGKOP
      RETURN
      END
