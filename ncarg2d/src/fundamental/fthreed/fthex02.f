...
C
 ...
 108  CONTINUE
C
C Draw a wire-frame representation of the surface defined by the
C function WFUN, using the routines FRST3 and VECT3.
C
      DO 110 I=1,11
         UTMP=REAL(I-1)/10.
...
C 
 ...
 112  CONTINUE
C
C Double the line width and put a little set of axes at each point on
C the surface.
C
      CALL PLOTIF (0.,0.,2)
      CALL GSLWSC (2.)
C
      DO 114 I=1,21,2
         ...
         DO 113 J=1,21,2
            VTMP=REAL(J-1)/20.
            WTMP=WFUN(UTMP,VTMP)
            CALL LINE3 (UTMP-.04,VTMP,WTMP,UTMP+.04,VTMP,WTMP)
            CALL LINE3 (UTMP,VTMP-.04,WTMP,UTMP,VTMP+.04,WTMP)
            CALL LINE3 (UTMP,VTMP,WTMP-.04,UTMP,VTMP,WTMP+.04)
 113     CONTINUE
 114  CONTINUE
C
C Advance the frame.
