C
C $Id: cpcldm.f,v 1.2 1994-03-17 01:50:34 kennison Exp $
C
...
10030 CONTINUE
        ...
        IF (JCCL.GE.0) THEN
          ...
          IF (IGER.NE.0) THEN
            CALL SETER ('CPCLDM - ERROR EXIT FROM GQPLCI',32,1)
            RETURN
          END IF
          CALL GQTXCI (IGER,ISTC)
          IF (IGER.NE.0) THEN
            CALL SETER ('CPCLDM - ERROR EXIT FROM GQTXCI',33,1)
            RETURN
          END IF
          CALL GSPLCI (JCCL)
          CALL GSTXCI (JCCL)
        ...
        IF (CLWS.GT.0.) THEN
          ...
          IF (IGER.NE.0) THEN
            CALL SETER ('CPCLDM - ERROR EXIT FROM GQLWSC',34,1)
            RETURN
          END IF
          CALL GSLWSC (CLWS)
        END IF
