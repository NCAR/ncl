C
C	$Id: cpcldr.f,v 1.1.1.1 1992-04-17 22:32:43 ncargd Exp $
C
C
C-----------------------------------------------------------------------
C
...
C
      IF (INIT.EQ.0) THEN
        CALL SETER ('CPCLDR - INITIALIZATION CALL NOT DONE',1,2)
        STOP
      END IF
C
...
10052   CONTINUE
          ...
          CALL CPDRSG (RWRK,IRW1,IRW2,NRWK)
          IF (IHCF.NE.0) CALL CPHCHR (RWRK,IRW1,IRW2,NRWK)
        GO TO 10052
10053   CONTINUE
...
10029 CONTINUE
        ...
        IF (JCCL.GE.0) THEN
          ...
          IF (IGER.NE.0) THEN
            CALL SETER ('CPCLDR - ERROR EXIT FROM GQPLCI',2,2)
            STOP
          END IF
          CALL GQTXCI (IGER,ISTC)
          IF (IGER.NE.0) THEN
            CALL SETER ('CPCLDR - ERROR EXIT FROM GQTXCI',3,2)
            STOP
          END IF
          CALL GSPLCI (JCCL)
          CALL GSTXCI (JCCL)
        ...
        IF (CLWS.GT.0.) THEN
          ...
          IF (IGER.NE.0) THEN
            CALL SETER ('CPCLDR - ERROR EXIT FROM GQLWSC',4,2)
            STOP
          END IF
          CALL GSLWSC (CLWS)
        END IF
