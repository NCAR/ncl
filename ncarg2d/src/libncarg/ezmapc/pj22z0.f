C
C $Id: pj22z0.f,v 1.1 1999-04-02 23:06:03 kennison Exp $
C
...
C
         SP=SIN(LAT)
         PHIDP=ASINZ0(((ONE-ES)*CA*SP-SA*COS(LAT)*SIN(LAMT))/SQRT(ONE
     .     -ES*SP*SP))
C
C        COMPUTE X AND Y
C
...
C        COMPUTE GEODETIC LATITUDE.
C
         IF (ABS(SA).LT.TOL) LAT=ASINZ0(SPP/SQRT((ONE-ES)*(ONE-ES)
     .      +ES*SPPSQ))
