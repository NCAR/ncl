C
C $Id: pjinit.f,v 1.2 1999-04-08 23:30:24 kennison Exp $
C
...
C
      ...
      INTEGER I,ZONE,DEGS(5),MINS(5)
      INTEGER ID, IND, ITEM, ITYPE, MODE, N
      INTEGER ISPHER, LUNIT, NAD27(134), NAD83(134)
C
      COMMON /SPLANE/ ID27,ID83,NM27,NM83,PA27,PA83
...
C
         IF (ITYPE .EQ. 1) THEN
            DATA(3) = TABLE(4)
            DATA(5) = PAKRZ0(PAKCZ0(TABLE(3)))/.017453292519943D0
            DATA(6) = PAKRZ0(PAKCZ0(TABLE(7)))/.017453292519943D0
            DATA(7) = TABLE(8)
            DATA(8) = TABLE(9)
            GO TO 900
         END IF
...
C
         IF (ITYPE .EQ. 2) THEN
            DATA(3) = PAKRZ0(PAKCZ0(TABLE(6)))/.017453292519943D0
            DATA(4) = PAKRZ0(PAKCZ0(TABLE(5)))/.017453292519943D0
            DATA(5) = PAKRZ0(PAKCZ0(TABLE(3)))/.017453292519943D0
            DATA(6) = PAKRZ0(PAKCZ0(TABLE(7)))/.017453292519943D0
            DATA(7) = TABLE(8)
            DATA(8) = TABLE(9)
...
C
         IF (ITYPE .EQ. 3) THEN
            DATA(5) = PAKRZ0(PAKCZ0(TABLE(3)))/.017453292519943D0
            DATA(6) = PAKRZ0(PAKCZ0(TABLE(4)))/.017453292519943D0
            DATA(7) = TABLE(5)
            DATA(8) = TABLE(6)
            GO TO 700
         END IF
...
C
         IF (ITYPE .EQ. 4) THEN
            DATA(3) = TABLE(4)
            DATA(4) = PAKRZ0(PAKCZ0(TABLE(6)))/.017453292519943D0
            DATA(5) = PAKRZ0(PAKCZ0(TABLE(3)))/.017453292519943D0
            DATA(6) = PAKRZ0(PAKCZ0(TABLE(7)))/.017453292519943D0
            DATA(7) = TABLE(8)
            DATA(8) = TABLE(9)
 ...
 2050    FORMAT (/' INITIALIZATION PARAMETERS (OBLIQUE MERCATOR ''HOTI',
     ...
     .            ' FALSE NORTHING               =',F12.2,' METERS')
         IF (MODE .NE. 0) THEN
            ...
            G = HALF * (F - ONE / F)
            GAMMA = ASINZ0 (SIN (ALPHA) / D)
            LON020 = LONC - ASINZ0 (G * TAN (GAMMA)) / BL
C
