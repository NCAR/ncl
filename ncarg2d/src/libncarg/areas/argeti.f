C
C	$Id: argeti.f,v 1.1.1.1 1992-04-17 22:32:12 ncargd Exp $
C
C
...
C
      IF (.NOT.(IPN(1:2).EQ.'AT')) GO TO 10002
        IF (IAU.EQ.0) CALL ARINIT
        IVL=IAU
        IF (IVL.GE.3) IVL=IBS
      GO TO 10003
10002 CONTINUE
      IF (.NOT.(IPN(1:2).EQ.'DB')) GO TO 10004
        IVL=IDB
      GO TO 10003
10004 CONTINUE
      IF (.NOT.(IPN(1:2).EQ.'DC')) GO TO 10005
        IVL=IDC
      GO TO 10003
10005 CONTINUE
      IF (.NOT.(IPN(1:2).EQ.'DI')) GO TO 10006
        IVL=IDI
      GO TO 10003
10006 CONTINUE
      IF (.NOT.(IPN(1:2).EQ.'LC')) GO TO 10007
        IVL=ILC
      GO TO 10003
10007 CONTINUE
