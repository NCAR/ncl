C
C	$Id: mapbd.f,v 1.1.1.1 1992-04-17 22:31:56 ncargd Exp $
C
C
C***********************************************************************
...
C
C September, 1986  Added SAVE statements for COMMON blocks.
C
C October, 1986    Corrected the use of the parameter DATL to prevent
C                  a blowup when using "SET=INDEF".
C
C November, 1986   Corrected the code setting IMF and IPF, in MAPGRD.
C
C April, 1987      Added code for the interface with MAPLAM and changed
C                  the way in which color indices are handled.  Put in
C                  copyrights and arranged for the card-image dataset
C                  to be copyrighted.
C
C April, 1991      Added the routine MAPTRA, which calls MAPTRN, but
C                  then checks the returned values to see if they lie
C                  outside the perimeter, and, if so, substitutes a
C                  1.E12 for UVAL.
C
C August, 1991     Changed MAPTRA to use a perimeter slightly outside
C                  the "actual" perimeter.  This is sort of a kludge;
C                  it reduces the probability that one edge of a user's
C                  grid will map precisely on top of the boundary of
C                  visibility.
C
C September, 1991  Changed key sections of MAPINT and MAPTRN to use
C                  double precision.  On small machines, inaccuracies
C                  at the point opposite the center of the projection
C                  were very large and caused serious problems for
C                  EZMAPA.  Also changed the code that interpolates
C                  points along great-circle paths in regions of great
C                  distortion to use fewer points.
C
C October, 1991    Changed the code to use double precision only on
C                  machines on which it is really necessary.  On the
C                  Y-MP, in particular, using double precision was
C                  very expensive.
C
      END
