C
C $Id: mapit.f,v 1.8 1999-04-02 22:59:34 kennison Exp $
C
...
C
     ...
     +                 RLTE,RLNE,PAPE,UCOE,VCOE)
          ...
          CALL MAPVP  (UOLD,VOLD,UCOE,VCOE)
...
C
     ...
     +             RLTO,RLNO,POLD,UOLD,VOLD,
     +             RLTO,RLNO,POLD,UOLD,VOLD)
      IF (ICFELL('MAPIT',11).NE.0) RETURN
C
...
C
      ...
      IF (IDTL.EQ.0) THEN
        CALL FRSTD (UOLD,VOLD)
        IF (ICFELL('MAPIT',12).NE.0) RETURN
      ELSE
        DATL=0.
      END IF
C
...
C
  102 IF ((IOPI.LE.NOPI.OR.IFST.LT.2).AND.
     +    ((UNEW-UOLD)**2+(VNEW-VOLD)**2)*DSSQ.LE.DPSQ)GO TO 104
      CALL MAPVP (UOLD,VOLD,UNEW,VNEW)
      IF (ICFELL('MAPIT',13).NE.0) RETURN
C
