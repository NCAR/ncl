...
/*
 * Declare space to hold a pointer to a gray-scale array and the dimensions
 * of the array.
 */
...
/*
 * GSVINI is called by EditEzmapData to read the PNG file whose name is
 * "fnmp" and extract from it a gray-scale array of dimension mgsv x nsgv,
 * which is placed in memory allocated by "calloc".  If an error occurs,
 * "ierr" is returned non-zero.
 */

void NGCALLF(gsvini,GSVINI)(fnmp,mgsv,ngsv,ired,igrn,qchr,ierr)
  char *fnmp;
  int  *mgsv;
  int  *ngsv;
  int  *ired;
  ...
  int  *ierr;
{
  ...
  *mgsv=idim;
  *ngsv=jdim;
  gsva=(unsigned char*)calloc((size_t)(idim*jdim),sizeof(char));
