C
C	$Id: sfnorm.f,v 1.1.1.1 1992-04-17 22:32:54 ncargd Exp $
C
C
C-----------------------------------------------------------------------
C
...
C
     ...
     +         ( XRA(IP1)*YRA(IP2)-XRA(IP2)*YRA(IP1) ) ) /
     +         ( XCO * ( XRA(IP2)-XRA(IP1) )
     +         + YCO * ( YRA(IP2)-YRA(IP1) ) )
10033 CONTINUE
      GO TO 10034
...
C
      IN1=NRA+1
      IF (.NOT.(LPA.EQ.0)) GO TO 10041
      IF (.NOT.(ABS(XCO).GT.ABS(YCO))) GO TO 10042
10043 CONTINUE
      IF (.NOT.(IN1.LT.LND)) GO TO 10044
      JN1=NRA+IND(IN1)
      IN2=IN1+1
10045 CONTINUE
      JN2=NRA+IND(IN2)
      IF (IN2.GE.LND) GO TO 10046
      JNT=NRA+IND(IN2+1)
      IF (DST(JNT)-DST(JN2).GT..000001) GO TO 10046
      IN2=IN2+2
      GO TO 10045
10046 CONTINUE
      IF (.NOT.(DST(JN2)-DST(JN1).GT..000001)) GO TO 10047
...
10042 CONTINUE
10049 CONTINUE
      IF (.NOT.(IN1.LT.LND)) GO TO 10050
      JN1=NRA+IND(IN1)
      IN2=IN1+1
10051 CONTINUE
      JN2=NRA+IND(IN2)
      IF (IN2.GE.LND) GO TO 10052
      JNT=NRA+IND(IN2+1)
      IF (DST(JNT)-DST(JN2).GT..000001) GO TO 10052
      IN2=IN2+2
      GO TO 10051
10052 CONTINUE
      IF (.NOT.(DST(JN2)-DST(JN1).GT..000001)) GO TO 10053
...
10048 CONTINUE
      GO TO 10054
10041 CONTINUE
      IF (.NOT.(ABS(XCO).GT.ABS(YCO))) GO TO 10055
10056 CONTINUE
      IF (.NOT.(IN1.LT.LND)) GO TO 10057
      JN1=NRA+IND(IN1)
      IN2=IN1+1
10058 CONTINUE
      JN2=NRA+IND(IN2)
      IF (IN2.GE.LND) GO TO 10059
      JNT=NRA+IND(IN2+1)
      IF (DST(JNT)-DST(JN2).GT..000001) GO TO 10059
      IN2=IN2+2
      GO TO 10058
10059 CONTINUE
      IF (.NOT.(DST(JN2)-DST(JN1).GT..000001)) GO TO 10060
...
10061 CONTINUE
      JSP =JSP +1
10063 CONTINUE
      IF (JSP .GT.(JS2)) GO TO 10062
      IMP=MOD(JSP,8)+1
      IF (IMP.LE.0) IMP=IMP+8
      IF (.NOT.(LDP(IMP,JMP).NE.0)) GO TO 10064
      SPJ=REAL(JSP)*SBL
      NPT=NPT+1
      XPT(NPT)=XCO*DFB+YCO*SPJ
      YPT(NPT)=YCO*DFB-XCO*SPJ
      IF (.NOT.(NPT.EQ.100)) GO TO 10065
...
10055 CONTINUE
10067 CONTINUE
      IF (.NOT.(IN1.LT.LND)) GO TO 10068
      JN1=NRA+IND(IN1)
      IN2=IN1+1
10069 CONTINUE
      JN2=NRA+IND(IN2)
      IF (IN2.GE.LND) GO TO 10070
      JNT=NRA+IND(IN2+1)
      IF (DST(JNT)-DST(JN2).GT..000001) GO TO 10070
      IN2=IN2+2
      GO TO 10069
10070 CONTINUE
      IF (.NOT.(DST(JN2)-DST(JN1).GT..000001)) GO TO 10071
...
10072 CONTINUE
      JSP =JSP +1
10074 CONTINUE
      IF (JSP .GT.(JS2)) GO TO 10073
      IMP=MOD(JSP,8)+1
      IF (IMP.LE.0) IMP=IMP+8
      IF (.NOT.(LDP(IMP,JMP).NE.0)) GO TO 10075
      SPJ=REAL(JSP)*SBL
      NPT=NPT+1
      XPT(NPT)=XCO*DFB+YCO*SPJ
      YPT(NPT)=YCO*DFB-XCO*SPJ
      IF (.NOT.(NPT.EQ.100)) GO TO 10076
      CALL POINTS (XPT,YPT,100,LCH,0)
      NPT=0
10076 CONTINUE
10075 CONTINUE
...
C
10020 CONTINUE
      IF (.NOT.(KND.LE.NND)) GO TO 10079
      DO 10080 I=KND,NND
      IF (.NOT.(IND(I).EQ.IPX)) GO TO 10081
      IND(I)=IND(KND)
      KND=KND+1
      GO TO 101
10081 CONTINUE
