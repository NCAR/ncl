...
C
	...
	CALL PLCHMQ (.50,.95,
     ...
     +                                                       .015,0.,0.)
	CALL PLCHMQ (.50,.92,'(USED IN CALLS TO GKS ROUTINES LIKE GCA, G
     +FA, GPL, GPM, AND GTX)',
     +                                                       .012,0.,0.)
	CALL PLCHMQ (.50,.15,
  ...
  101   CONTINUE
	...
	CALL PLCHMQ (CFUX(CUFX(100.)+.005),CFUY(CUFY(1000.)-.01),
     +               'VIEWPORT',.01,0.,-1.)
	CALL PLCHMQ (100.,CFUY(CUFY(100.)-.01),'.15',.008,0.,0.)
	CALL PLCHMQ (1000.,CFUY(CUFY(100.)-.01),'.95',.008,0.,0.)
	CALL PLCHMQ (CFUX(CUFX(100.)-.005),100.,'.10',.008,0.,1.)
	CALL PLCHMQ (CFUX(CUFX(100.)-.005),1000.,'.90',.008,0.,1.)
	CALL CURVE  (XCRA,YCRA,101)
	XCLV=CUFX(100.)
	XCRV=CUFX(1000.)
	YCBV=CUFY(100.)
