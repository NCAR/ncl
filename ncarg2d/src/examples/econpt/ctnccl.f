...
C
        CALL GTNCCL (RPNT,MPNT,NPNT,LOPN,  !  point list
...
C       CALL GSCR   (IWID, 2,1.,1.,0.)  !  yellow
        CALL GSCR   (IWID, 2,.8,.8,.8)  !  gray (for outside-mesh area)
...
C
        DO 101 I=1,NOCL
          ...
          IF (ICLU(I).EQ.1) THEN
            CALL CTSETI ('CLC - CONTOUR LEVEL COLOR',9)  !  light gray
          ELSE
            CALL CTSETI ('CLC - CONTOUR LEVEL COLOR',1)  !  black
          END IF
  101   CONTINUE
C
...
C
        DO 103 IDIR=1,1  !  one (OR) views of the globe
C
          PRINT * , ' '
