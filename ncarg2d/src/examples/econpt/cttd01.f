...
C
C Tell CONPACKT to use a little more workspace in CTTDBI.
C
...
C
          CALL CTSETI ('TBM - TRIANGLE BLOCKING MASK',1)
C
...
C
C Set flags to block invisible triangles from this view.
C
          PRINT * , '  CALLING CTTDBI'
C
          CALL CTTDBI (RPNT,IEDG,ITRI,RWRK,IWRK)
C
...
C
            IF (IDIA.EQ.0.AND.IEDG(IPTE+6).NE.0) GO TO 105
C
            IFLL=0
C
            IF (IEDG(IPTE+3).GE.0) THEN
              IF (IAND(ITRI(LOTN*((IEDG(IPTE+3)-1)/LOTN)+4),ITBM)
     +                                                     .EQ.0) IFLL=1
            END IF
C
            IFLR=0
C
            IF (IEDG(IPTE+4).GE.0) THEN
              IF (IAND(ITRI(LOTN*((IEDG(IPTE+4)-1)/LOTN)+4),ITBM)
     +                                                     .EQ.0) IFLR=1
            END IF
C
...
C
     ...
     +                 IWRK,LIWK)       !  integer workspace
C
C Set the triangle blocking mask to block triangles not visible from
C this view.
C
          CALL CTSETI ('TBM - TRIANGLE BLOCKING MASK',ITBM)
C
          IF (IMSN.NE.0) THEN
C
            PRINT * , '  DRAWING TRIANGULAR MESH'
...
C
              IF (IDIA.EQ.0.AND.IEDG(IPTE+6).NE.0) GO TO 107
C
              IFLL=0
C
              IF (IEDG(IPTE+3).GE.0) THEN
...
C
            ...
            CALL GSPLCI (8)
C
            DO 109 IPTE=0,NEDG-LOEN,LOEN
C
              IF (IDIA.EQ.0.AND.IEDG(IPTE+6).NE.0) GO TO 109
C
              IFLL=0
C
              IF (IEDG(IPTE+3).GE.0) THEN
...
C
            ...
            CALL GSPLCI (8)
C
            DO 111 IPTE=0,NEDG-LOEN,LOEN
C
              IF (IDIA.EQ.0.AND.IEDG(IPTE+6).NE.0) GO TO 111
C
              IFLL=0
C
              IF (IEDG(IPTE+3).GE.0) THEN
