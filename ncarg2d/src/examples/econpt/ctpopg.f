...
C
        CALL GTPOPG (RPNT,MPNT,NPNT,LOPN,  !  point list
...
C
              CALL PLCHHQ (CFUX(.03),CFUY(.946),'POP GRID',.024,0.,-1.)
C
              IF      (IDIR.EQ.1) THEN
...
C
      ...
      SUBROUTINE GTPOPG (RPNT,MPNT,NPNT,LOPN,
     +                   IEDG,MEDG,NEDG,LOEN,
     +                   ITRI,MTRI,NTRI,LOTN,
     +                             CLAT,CLON)
C
...
C
C The POP grid is rectangular; at each point, we have a latitude, a
C longitude, and a temperature.  Temperatures over land have a special
C value that marks them as "missing".  From this, we construct a
C triangular mesh.  First, define dimensions:
C
        PARAMETER (IDIM=101,JDIM=116)
        PARAMETER (IDM1=IDIM-1,JDM1=JDIM-1)
C
C Dimension arrays to receive the original POP grid data.
C
        DIMENSION TLAT(IDM1,JDIM),TLON(IDM1,JDIM)
        DIMENSION ULAT(IDM1,JDIM),ULON(IDM1,JDIM)
        DIMENSION TEMP(IDM1,JDIM)
C
C Declare arrays to receive a modified form of the POP grid data.  XLAT
C is the array of latitudes, XLON the array of longitudes, and ZDAT the
C array of sea surface temperatures.  Each of these arrays has one point
C of overlap in the first dimension: for each value of J from 1 to JDIM,
C a value indexed [1,J] is the same as a value indexed [IDIM,J].
C
        COMMON /CMPOPG/ XLAT(IDIM,JDIM),XLON(IDIM,JDIM),ZDAT(IDIM,JDIM)
        SAVE   /CMPOPG/
C
...
C
        EXTERNAL MIPOPG
C
C Define the value that indicates a missing datum in the POP grid.
C
        DATA SVAL / -99. /
C
...
C
C       ISTA=NF_OPEN('ctpopg.nc',0,NCID)
C
...
C       END IF
C
C Read the array of latitudes of cell boundary points.
C
...
C       END IF
C
C Read the array of longitudes of cell boundary points.
C
...
C       END IF
C
C Read the array of temperatures.
C
...
C were used to create a "fort.11" which was then renamed "ctpopg.dat".)
C
...
C
C Read the required data from the ASCII file "ctpopg.dat".
C
        OPEN (11,FILE='ctpopg.dat',STATUS='OLD',FORM='FORMATTED')
C
        READ (11,'(5E16.8)') TLAT
        READ (11,'(5E16.8)') TLON
        READ (11,'(5E16.8)') ULAT
...
C
        CLOSE (11)
C
C Define the contents of the arrays XLAT, XLON, and ZDAT, using one
C point of overlap in the "I" subscript.  Note that we are using the
C positions of the center points of POP grid cells as the positions of
C the corner points of the rectangular grid from which we will make up
C the triangular mesh.  Note also that, instead of using all the center-
C point positions from the NetCDF file, we compute those for J.GT.100
C ourselves, thus fixing a problem caused by the fact that SCRIP was
C used to compute them, in a manner which failed badly near the North
C Pole.
C
        DO 102 I=1,IDIM
          III=MOD(I+IDM1-1,IDM1)+1
          IM1=MOD(I+IDM1-2,IDM1)+1
          DO 101 J=1,JDIM
            IF (J.LE.100) THEN
              XLAT(I,J)=TLAT(III,J)
              XLON(I,J)=TLON(III,J)
            ELSE
              CALL IPIQOS (ULAT(IM1,J-1),ULON(IM1,J-1),
     +                     ULAT(III,J-1),ULON(III,J-1),
     +                     ULAT(IM1,J  ),ULON(IM1,J  ),
     +                     ULAT(III,J  ),ULON(III,J  ),
     +                                        .5,.5,XLAT(I,J),XLON(I,J))
            END IF
            ZDAT(I,J)=TEMP(III,J)
  101     CONTINUE
  102   CONTINUE
...
C equivalent to it.
C
        CALL CTTMRG (IDIM,JDIM,XLAT,XLON,ZDAT,ISCR,SVAL,MIPOPG,
     +               RPNT,MPNT,NPNT,LOPN,
     +               IEDG,MEDG,NEDG,LOEN,
     +               ITRI,MTRI,NTRI,LOTN)
C
...
C
      ...
      SUBROUTINE MIPOPG (IDIM,JDIM,IINI,JINI,IINO,JINO)
C
...
C
        IF (IINI.EQ.IDIM) THEN  !  point in last column of grid
          IINO=1
          JINO=JINI
...
C
      ...
      SUBROUTINE DSPOPG
C
C Draw the original POP grid, using data passed in a common block.
C
        PARAMETER (IDIM=101,JDIM=116,IDM1=IDIM-1,JDM1=JDIM-1)
C
        COMMON /CMPOPG/ XLAT(IDIM,JDIM),XLON(IDIM,JDIM),ZDAT(IDIM,JDIM)
...
C
        CALL SHDGRD (.03,.33,.03,.37,IDM1,JDM1,3,7)
C
C Done.
C
        RETURN
