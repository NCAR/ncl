...
C the use of a simple, closed, lat/lon grid on the surface of the globe.
...
C
        PARAMETER (IDIM=181,JDIM=91,IDM1=IDIM-1,JDM1=JDIM-1)
C
...
C
        CALL GTLLG3 (RPNT,MPNT,NPNT,LOPN,  !  point list
...
C
              CALL PLCHHQ (CFUX(.03),CFUY(.946),'LAT/LON GRID',
     +                                                      .024,0.,-1.)
              CALL PLCHHQ (CFUX(.03),CFUY(.910),'(COMPLETELY CLOSED)',
     +                                                      .012,0.,-1.)
              IF      (IDIR.EQ.1) THEN
                CALL PLCHHQ (CFUX(.52),CFUY(.91),
     +                     'Both the left and right edges of',
     +                                                      .010,0.,-1.)
                CALL PLCHHQ (CFUX(.52),CFUY(.89),
     +                     'the grid map into a "seam" along',
     +                                                      .010,0.,-1.)
                CALL PLCHHQ (CFUX(.52),CFUY(.87),
     +                     'the International Date Line.',
     +                                                      .010,0.,-1.)
                CALL PLCHHQ (CFUX(.48),CFUY(.79),
     +                       'The entire top edge of the grid',
     +                                                       .010,0.,1.)
                CALL PLCHHQ (CFUX(.48),CFUY(.77),
     +                       'maps to a point at the North Pole.',
     +                                                       .010,0.,1.)
              ELSE IF (IDIR.EQ.2) THEN
                CALL PLCHHQ (CFUX(.52),CFUY(.13),
     +                     'Both the left and right edges of',
     +                                                      .010,0.,-1.)
                CALL PLCHHQ (CFUX(.52),CFUY(.11),
     +                       'the grid map into a "seam" along',
     +                                                      .010,0.,-1.)
                CALL PLCHHQ (CFUX(.52),CFUY(.09),
     +                       'the International Date Line.',
     +                                                      .010,0.,-1.)
                CALL PLCHHQ (CFUX(.52),CFUY(.23),
     +                       'The entire bottom edge of the grid',
     +                                                      .010,0.,-1.)
                CALL PLCHHQ (CFUX(.52),CFUY(.21),
     +                       'maps to a point at the South Pole.',
     +                                                      .010,0.,-1.)
              ELSE IF (IDIR.EQ.3) THEN
                CALL PLCHHQ (CFUX(.53),CFUY(.22),
     +                       'Both the left and right edges of',
     +                                                      .010,0.,-1.)
                CALL PLCHHQ (CFUX(.53),CFUY(.20),
     +                       'the grid map into a "seam" along',
     +                                                      .010,0.,-1.)
                CALL PLCHHQ (CFUX(.53),CFUY(.18),
     +                       'the International Date Line.',
     +                                                      .010,0.,-1.)
                CALL PLCHHQ (CFUX(.48),CFUY(.79),
     +                       'The entire top edge of the grid',
     ...
     +                                                       .010,0.,1.)
              ELSE IF (IDIR.EQ.4) THEN
                CALL PLCHHQ (CFUX(.53),CFUY(.85),
     +                       'Both the left and right edges of',
     +                                                      .010,0.,-1.)
                CALL PLCHHQ (CFUX(.53),CFUY(.83),
     +                       'the grid map into a "seam" along',
     +                                                      .010,0.,-1.)
                CALL PLCHHQ (CFUX(.53),CFUY(.81),
     +                       'the International Date Line.',
     +                                                      .010,0.,-1.)
                CALL PLCHHQ (CFUX(.48),CFUY(.23),
     +                       'The entire bottom edge of the grid',
     +                                                       .010,0.,1.)
                CALL PLCHHQ (CFUX(.48),CFUY(.21),
     +                       'maps to a point at the South Pole.',
     +                                                       .010,0.,1.)
              END IF
C
...
C
      ...
      SUBROUTINE GTLLG3 (RPNT,MPNT,NPNT,LOPN,
     +                   IEDG,MEDG,NEDG,LOEN,
     +                   ITRI,MTRI,NTRI,LOTN,
     +                             CLAT,CLON)
C
...
C
C Declare arrays to receive the grid data.  XLAT is the array of
C latitudes, XLON the array of longitudes, and ZDAT the array of
C field values.
C
        COMMON /CMLLG3/ XLAT(IDIM,JDIM),XLON(IDIM,JDIM),ZDAT(IDIM,JDIM)
        SAVE   /CMLLG3/
C
...
C equivalent to it.
C
        CALL CTTMRG (IDIM,JDIM,XLAT,XLON,ZDAT,ISCR,SVAL,MILLG3,
     +               RPNT,MPNT,NPNT,LOPN,
     +               IEDG,MEDG,NEDG,LOEN,
     +               ITRI,MTRI,NTRI,LOTN)
C
...
C
      ...
      SUBROUTINE MILLG3 (IDIM,JDIM,IINI,JINI,IINO,JINO)
C
...
C       CALL GSFACI (0)
C
C       IF (MOD(IDIM,2).EQ.0) THEN
C         CALL FPSGCR (XLAT(IDIM/2  ,JDIM),XLON(IDIM/2  ,JDIM),
C    +                 XLAT(IDIM/2+1,JDIM),XLON(IDIM/2+1,JDIM),
C    +                                                     .5,CLAT,CLON)
C       ELSE
C         CLAT=XLAT(IDIM/2+1,JDIM)
C         CLON=XLON(IDIM/2+1,JDIM)
C       END IF
C       CALL MAPTRA (CLAT,CLON,XVAL,YVAL)
C       IF (XVAL.NE.1.E12.AND.YVAL.NE.1.E12) THEN
C         CALL ELLIPS (CUFX(XVAL),CUFY(YVAL),RADA,RADB,0.,4.)
C       END IF
C
C       IF (MOD(JDIM,2).EQ.0) THEN
C         CALL FPSGCR (XLAT(1,JDIM/2  ),XLON(1,JDIM/2  ),
C    +                 XLAT(1,JDIM/2+1),XLON(1,JDIM/2+1),
C    +                                                     .5,CLAT,CLON)
C       ELSE
C         CLAT=XLAT(1,JDIM/2+1)
C         CLON=XLON(1,JDIM/2+1)
C       END IF
C       CALL MAPTRA (CLAT,CLON,XVAL,YVAL)
C       IF (XVAL.NE.1.E12.AND.YVAL.NE.1.E12) THEN
C         CALL ELLIPS (CUFX(XVAL),CUFY(YVAL),RADA,RADB,0.,4.)
...
C
C       IF (MOD(JDIM,2).EQ.0) THEN
C         CALL FPSGCR (XLAT(IDIM,JDIM/2  ),XLON(IDIM,JDIM/2  ),
C    +                 XLAT(IDIM,JDIM/2+1),XLON(IDIM,JDIM/2+1),
C    +                                                     .5,CLAT,CLON)
C       ELSE
C         CLAT=XLAT(IDIM,JDIM/2+1)
C         CLON=XLON(IDIM,JDIM/2+1)
C       END IF
C       CALL MAPTRA (CLAT,CLON,XVAL,YVAL)
C       IF (XVAL.NE.1.E12.AND.YVAL.NE.1.E12) THEN
C         CALL ELLIPS (CUFX(XVAL),CUFY(YVAL),RADA,RADB,0.,4.)
...
C
C       IF (MOD(IDIM,2).EQ.0) THEN
C         CALL FPSGCR (XLAT(IDIM/2  ,1),XLON(IDIM/2  ,1),
C    +                 XLAT(IDIM/2+1,1),XLON(IDIM/2+1,1),
C    +                                                     .5,CLAT,CLON)
C       ELSE
C         CLAT=XLAT(IDIM/2+1,1)
C         CLON=XLON(IDIM/2+1,1)
C       END IF
C       CALL MAPTRA (CLAT,CLON,XVAL,YVAL)
C       IF (XVAL.NE.1.E12.AND.YVAL.NE.1.E12) THEN
C         CALL ELLIPS (CUFX(XVAL),CUFY(YVAL),RADA,RADB,0.,4.)
...
C
        CALL SHDGRD (.03,.33,.03,.18,IDM1,JDM1,3,5)
C
C Done.
C
        RETURN
