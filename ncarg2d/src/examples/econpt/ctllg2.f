...
C the use of a simple, nearly closed, lat/lon grid on the surface of the
...
C
        PARAMETER (IDIM=171,JDIM=81,IDM1=IDIM-1,JDM1=JDIM-1)
C
...
C
        CALL GTLLG2 (RPNT,MPNT,NPNT,LOPN,  !  point list
...
C beused in making an illustrative drawing of the rectangular grid.
C
...
C
     ...
     +                                                      .024,0.,-1.)
              CALL PLCHHQ (CFUX(.03),CFUY(.910),'(NEARLY CLOSED)',
     +                                                      .012,0.,-1.)
C
              IF      (IDIR.EQ.1) THEN
...
C
      ...
      SUBROUTINE GTLLG2 (RPNT,MPNT,NPNT,LOPN,
     +                   IEDG,MEDG,NEDG,LOEN,
     +                   ITRI,MTRI,NTRI,LOTN,
     +                             CLAT,CLON)
C
...
C field values.
C
        COMMON /CMLLG2/ XLAT(IDIM,JDIM),XLON(IDIM,JDIM),ZDAT(IDIM,JDIM)
        SAVE   /CMLLG2/
C
...
C
        DO 102 I=1,IDIM
          DO 101 J=1,JDIM
            ...
            XLON(I,J)=-170.+(REAL(I-1)/REAL(IDIM-1))*340.
...
C equivalent to it.
C
        CALL CTTMRG (IDIM,JDIM,XLAT,XLON,ZDAT,ISCR,SVAL,MILLG2,
     +               RPNT,MPNT,NPNT,LOPN,
     +               IEDG,MEDG,NEDG,LOEN,
     +               ITRI,MTRI,NTRI,LOTN)
C
...
C
      ...
      SUBROUTINE MILLG2 (IDIM,JDIM,IINI,JINI,IINO,JINO)
C
...
C
        CALL SHDGRD (.03,.33,.03,.17,IDM1,JDM1,3,7)
C
C Done.
C
        RETURN
