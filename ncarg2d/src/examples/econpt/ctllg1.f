...
C the use of a simple, unclosed, lat/lon grid on the surface of the
C globe.  The triangular mesh is created from the rectangular grid by
C calling the CONPACKT routine CTTMRG.
C
...
C
        CALL GTLLG1 (RPNT,MPNT,NPNT,LOPN,  !  point list
...
C
     ...
     +                                                      .024,0.,-1.)
              CALL PLCHHQ (CFUX(.03),CFUY(.910),'(UNCLOSED)',
     +                                                      .012,0.,-1.)
              IF      (IDIR.EQ.3) THEN
                CALL PLCHHQ (CFUX(.5),CFUY(.52),
     +                       'From this viewpoint, only the top',
     +                                                       .010,0.,0.)
                CALL PLCHHQ (CFUX(.5),CFUY(.50),
     +                       'corners of the grid are visible; the',
     +                                                       .010,0.,0.)
                CALL PLCHHQ (CFUX(.5),CFUY(.48),
     +                       ' rest of it is hidden by the globe.',
     +                                                       .010,0.,0.)
              ELSE IF (IDIR.EQ.4) THEN
                CALL PLCHHQ (CFUX(.5),CFUY(.52),
     +                       'From this viewpoint, only the bottom',
     +                                                       .010,0.,0.)
                CALL PLCHHQ (CFUX(.5),CFUY(.50),
     +                       'corners of the grid are visible; the',
     +                                                       .010,0.,0.)
...
C
      ...
      SUBROUTINE GTLLG1 (RPNT,MPNT,NPNT,LOPN,
     +                   IEDG,MEDG,NEDG,LOEN,
     +                   ITRI,MTRI,NTRI,LOTN,
     +                             CLAT,CLON)
C
...
C field values.
C
        COMMON /CMLLG1/ XLAT(IDIM,JDIM),XLON(IDIM,JDIM),ZDAT(IDIM,JDIM)
        SAVE   /CMLLG1/
C
...
C
        DO 102 I=1,IDIM
          DO 101 J=1,JDIM
            XLAT(I,J)=-40.+(REAL(J-1)/REAL(JDIM-1))*80.
            XLON(I,J)=-80.+(REAL(I-1)/REAL(IDIM-1))*160.
...
C equivalent to it.
C
        CALL CTTMRG (IDIM,JDIM,XLAT,XLON,ZDAT,ISCR,SVAL,MILLG1,
     +               RPNT,MPNT,NPNT,LOPN,
     +               IEDG,MEDG,NEDG,LOEN,
     +               ITRI,MTRI,NTRI,LOTN)
C
...
C
      ...
      SUBROUTINE MILLG1 (IDIM,JDIM,IINI,JINI,IINO,JINO)
C
...
C
      ...
      SUBROUTINE DCLLG1
C
C Draw a colored lat/lon grid, using data passed in a common block.
C
        PARAMETER (IDIM=81,JDIM=41,IDM1=IDIM-1,JDM1=JDIM-1)
C
        COMMON /CMLLG1/ XLAT(IDIM,JDIM),XLON(IDIM,JDIM),ZDAT(IDIM,JDIM)
...
C
        CALL SHDGRD (.03,.33,.03,.18,IDM1,JDM1,3,7)
C
C Done.
C
        RETURN
