/*
 *	$Id: c_tdashc.c,v 1.2 1994-06-21 14:59:53 haley Exp $
 */
#include <stdio.h>
#include <math.h>
...
{
/*
 * purpose                To provide a simple demonstration of DASHCHAR
 *
 * USAGE                  CALL TDASHC (IERROR)
 *
 * ARGUMENTS
 *
 * ON OUTPUT              IERROR
 ...
 *
 *               DASHCHAR TEST EXECUTED--SEE PLOT TO CERTIFY
 *
 ...
 * PRECISION              Single
 *
 * REQUIRED ROUTINES      DASHCHAR
 *
 * REQUIRED GKS LEVEL     0A
 *
 * LANGUAGE               FORTRAN 77
 *
 * ALGORITHM              TDASHC utilizes the software DASHCHAR
 *                        routines DASHDB, DASHDC, FRSTD, VECTD,
 *                        LINED, and CURVED to draw 5 curves on 1
 *                        picture using 5 different DASHCHAR patterns.
 *                        Each curve is centered about solid axis
 *                        lines and labelled with the character
 *                        representation of the DASHCHAR pattern used.
 *
 ...
 */
    ...
    text_pos.x = .5;
    text_pos.y = .985;
    gtext (&text_pos,"DEMONSTRATION PLOT FOR DASHCHAR");
    text_pos.y = .015;
    gtext (&text_pos,"IN IPAT STRINGS, A AND D SHOULD BE INTERPRETED;AS APOSTROPHE AND DOLLAR SIGN");
    c_frame();
    
    *ierror = 0;
    printf( "DASHCHAR TEST EXECUTED--SEE PLOT TO CERTIFY\n");
    return(1);
}
