 ...
 * Retrieve the values with which MAPINT called SET.
 */
            c_getset(&flew,&frew,&fbew,&ftew,&limits_.ulew,&limits_.urew,&limits_.vbew,&limits_.vtew,&lnlg);
            ilew=c_kfpx(flew);
            irew=c_kfpx(frew);
            ibew=c_kfpy(fbew);
            itew=c_kfpy(ftew);
 ...
 * labels.
 */
            limits_.delu=3.5*((float)limits_.icsz/(float)(irew-ilew))*(limits_.urew-limits_.ulew);
            limits_.delv=2.0*((float)limits_.icsz/(float)(itew-ibew))*(limits_.vtew-limits_.vbew);
            limits_.nsav=0;
/*
 * Draw the outlines.
 */
 ...
 */
    if (ucen >= limits_.ulew && ucen <= limits_.urew && 
        vcen >= limits_.vbew && vcen <= limits_.vtew) {
/*
 ...
 */
        x = .1428*limits_.delu*(float)nchs;
 ...
 */
        if ( ulab-ucen < 0 ) x = -(.1428*limits_.delu*(float)nchs);
        else                 x =   .1428*limits_.delu*(float)nchs;
        if ( vlab-vcen < 0 ) y = -(.3333*limits_.delv);
        else                 y =   .3333*limits_.delv;
        c_line (ucen,vcen,ulab-x,vlab-y);

        c_pwrit (ulab,vlab,csg1,nchs,limits_.icsz,0,0);
 ...
 */
    i = 0;
    do {
        if (fabs(*ulab-usav[i]) < limits_.delu && fabs(*vlab-vsav[i]) < limits_.delv) {
 ...
 */
            do {
                ign1=ign1+1;
                if (ign1 <= ign2/2) {
                    if ( -.5+(float)((ign2/2)%2) < 0 ) x = -limits_.delu;
                    else                               x =  limits_.delu;
                    *ulab = *ulab+x;
                }
                else {
                    if ( -.5+(float)((ign2/2)%2) < 0 ) y = -limits_.delv;
                    else                               y =  limits_.delv;
                    *vlab = *vlab+y;
                }
                if (ign1 == ign2) {
            ...
            } while(*ulab < limits_.ulew || *ulab > limits_.urew || 
                    *vlab < limits_.vbew || *vlab > limits_.vtew);
            i = -1;
        }
    } while( ++i < limits_.nsav );
/*
 * no overlap.  save the new label position and return to caller.
 */
    limits_.nsav++;
    if (limits_.nsav > 1000) return(0);
    usav[limits_.nsav-1] = *ulab;
    vsav[limits_.nsav-1] = *vlab;
    return(1);
}
