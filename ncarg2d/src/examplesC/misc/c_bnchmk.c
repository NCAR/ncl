...
**
**    The utilities AUTOGRPH, VELVCT, conrec, EZMAP, GRIDAL,
**    and DASHCHAR are required.  Since conrec and VELVCT plots
...
**     EXTERNAL FX, FY
**
**    This will force the functions fx and fy supplied in this
**    package to be loaded.  The documentation for fx explains
...
*/
	...
	for(j=0; j<3; ++j)
	{
      	...
      	c_frame();
/*
** Frame 2 -- conrec plot with polar coordinate Transformation.
*/
      	Trans = 2;
      	D1 = 20.0;
      	Ma = 30;
	...
	nhi = -1;
	ndot  = 0;
	NGCALLF(conrec,CONREC)(&data[3][0],&Ma,&Ma,&Na,&lo,&hi,&inc,&nset,&nhi,&ndot);
      	c_frame();

/*
** Frame 3 -- conrec overlaid on EZMAP.
*/
      	Ma = 55;
      	Na = 45;
      	Trans = 1;
	...
	nhi = 0;
	ndot  = -585;
	NGCALLF(conrec,CONREC)(&data[0][0],&Ma,&Ma,&Na,&lo,&hi,&inc,&nset,&nhi,&ndot);
      	c_frame();
/*
** Frame 4 -- conrec overlaid on EZMAP.
*/
      	Ma = 55;
      	Na = 45;
      	Trans = 1;
	...
	NGCALLF(conrec,CONREC)(&data[0][0],&Ma,&Ma,&Na,&lo,&hi,&inc,&nset,&nhi,&ndot);
      	c_frame();
/*
** Frame 5 --  conrec with modified background.
*/
      	Ma = 40;
      	Na = 40;
      	Trans = 0;
...
*/
	...
	*/
      	...
      	if (open == 0)
	{
		...
		*/
#if !defined (cray)
      		NGCALLF(gngpat,GNGPAT)(filenm,filenm2,&stat,119,9);
#else
          	cftfilenm = _cptofcd(filenm, strlen(filenm));
        	cftfilenm2 = _cptofcd(filenm2, strlen(filenm2));
        	NGCALLF(gngpat,GNGPAT)(cftfilenm,cftfilenm2,&stat);
...
float NGCALLF(fx,FX)(float* X, float* Y)
{
/*
**
...
float NGCALLF(fy,FY)(float* X, float* Y)
{
/*
**
** The function fy behaves in a similar manner to fx as
** described above.
*/
	extern int Ma;		/* number of data columns to be transformed */
	extern int Na;		/* number of data rows to be transformed */
