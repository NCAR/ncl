/*
 *	$Id: c_class3.c,v 1.1 1994-10-31 02:14:13 haley Exp $
 */
#include <stdio.h>
#include <math.h>
...
int NGCALLF(agchax,AGCHAX)(iflg,iaxs,iprt,vils)
int *iflg, *iaxs, *iprt;
float *vils;
{
	c_plotif (0.,0.,2);
...
int NGCALLF(agchcu,AGCHCU)(iflg,kdsh)
int *iflg, *kdsh;
{
	c_plotif (0.,0.,2);
	if (*iflg == 0) {
...
int NGCALLF(agchil,AGCHIL)(iflg,lbnm,lnno)
int *iflg, *lbnm, *lnno;
{
	c_plotif (0.,0.,2);
	if (*iflg == 0) {
...
int NGCALLF(agpwrt,AGPWRT)(xpos, ypos, chrs, nchs, isiz, iori, icen)
float *xpos, *ypos;
char *chrs;
