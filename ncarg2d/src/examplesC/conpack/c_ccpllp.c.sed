...
int main()
{
	...
	int ncons;		/* number of coutour levels */
...
/* Interpolate data onto a regular grid */
        c_idsfft(1,NRAN,xran,yran,zran,MREG,NREG,MREG,xreg,yreg,&zreg[0][0],iwrk,rwrk);

/* ------------Default Labels----------------- */
/* Set up viewport */
	c_cpsetr("VPT - VIEWPORT TOP",0.95);
	c_cpsetr("VPB - VIEWPORT BOTTOM",.50);
	c_cpsetr("VPL - VIEWPORT LEFT",0.0);
	c_cpsetr("VPR - VIEWPORT RIGHT",.48);
...
/* choose which labelling scheme will be used. */
	c_cpseti("LLP - LINE LABEL POSTIONING FLAG",1);
...
/* Title plot */
	gsel_norm_tran(0);
	c_pchiqu(.25, .975, "Default Labels", .017, 0., 0.);

/* ------------Medium Labels----------------- */
/* Set up Areas for drawing boxes */
	c_arinam(map, LMAP);
/* Set up viewport */
	c_cpsetr("VPT - VIEWPORT TOP",0.95);
	c_cpsetr("VPB - VIEWPORT BOTTOM",.50);
	c_cpsetr("VPR - VIEWPORT RIGHT",1.0);
	c_cpsetr("VPL - VIEWPORT LEFT",.52);
	c_cpsetc("ILT - INFORMATION LABEL TEXT"," ");
/* Initialize Conpack */
	c_cprect(&zreg[0][0], MREG, MREG, NREG, rwrk, LRWK, iwrk, LIWK);
/* Force Conpack to chose contour levels */
...
/* Modify Conpack chosen parameters */
	c_cpgeti("NCL - NUMBER OF CONTOUR LEVELS",&ncons);
/* choose which labelling scheme will be used. */
	c_cpseti("LLP - LINE LABEL POSTIONING FLAG",2);
/* Turn off high and low labels */
	c_cpsetc("HLT - HIGH/LOW LABEL TEXT"," "" ");
	for(i=1; i<=ncons; ++i)
	{
...
/* Title plot */
	gsel_norm_tran(0);
	c_pchiqu(.75, .975, "Regular Scheme", .017, 0., 0.);

/* ------------High Quality Labels----------------- */
/* Set up area map */
	c_arinam(map, LMAP);
/* Set up viewport */
	c_cpsetr("VPT - VIEWPORT TOP",.45);
	c_cpsetr("VPB - VIEWPORT BOTTOM",0.0);
	c_cpsetr("VPL - VIEWPORT LEFT",0.0);
	c_cpsetr("VPR - VIEWPORT RIGHT",1.0);
	c_cpsetc("ILT - INFORMATION LABEL TEXT"," ");
/* Initialize Conpack */
...
/* Modify Conpack chosen parameters */
	c_cpgeti("NCL - NUMBER OF CONTOUR LEVELS",&ncons);
/* choose which labelling scheme will be used. */
	c_cpseti("LLP - LINE LABEL POSTIONING FLAG",3);
/* Turn off high and low labels */
	c_cpsetc("HLT - HIGH/LOW LABEL TEXT"," "" ");
	for(i=1; i<=ncons; ++i)
	{
...
/* Title plot */
	gsel_norm_tran(0);
	c_pchiqu(.5, .475, "Penalty Scheme", .017, 0., 0.);
