/*
 *	$Id: c_cpex03.c,v 1.1 1994-10-31 02:26:26 haley Exp $
 */
#include <stdio.h>
#include <math.h>
...
} NGCALLF(cpmpc1,CPMPC1);

struct common2
{
    float xfij[33][33], yfij[33][33];
} NGCALLF(cpmpc2,CPMPC2);
 ...
 */
            ...
            c_set    (xvpl,xvpr,yvpb,yvpt,0.,1.,0.,1.,1);
            for( i = 0; i < 33; i++ ) {
                NGCALLF(cpmpc1,CPMPC1).xfoi[i]=(float)log10((double)(1.+9.*(float)i/32.));
            }
            for( j = 0; j < 33; j++ ) {
                NGCALLF(cpmpc1,CPMPC1).yfoj[j]=(float)log10((double)(1.+9.*(float)j/32.));
            }
/*
 ...
 */
            ...
            for( i = 0; i < 33; i++ ) {
                for( j = 0; j < 33; j++ ) {
                    NGCALLF(cpmpc2,CPMPC2).xfij[i][j]=(float)i/32.+((float)(16-i)/64.)*
                      ((float)(16-abs(j-16))/16.);
                    NGCALLF(cpmpc2,CPMPC2).yfij[i][j]=(float)j/32.+((float)(16-j)/64.)*
...
NGCALLF(cpmpxy,CPMPXY)(imap,xinp,yinp,xotp,yotp)
int *imap;
        ...
        if ( *imap == 2) {
            ...
            *yotp= *xinp*sin(.017453292519943 * *yinp);
            ...
            if ( *imap == 3) {
                ...
                *xotp = ((float)(i+1) - *xinp)*NGCALLF(cpmpc1,CPMPC1).xfoi[i-1]+
                        (*xinp-(float)i)*NGCALLF(cpmpc1,CPMPC1).xfoi[i];
                *yotp = ((float)(j+1) - *yinp)*NGCALLF(cpmpc1,CPMPC1).yfoj[j-1]+
                        (*yinp-(float)(j))*NGCALLF(cpmpc1,CPMPC1).yfoj[j];
            }
            else {
                if ( *imap == 4) {
                    i=max(1,min(32,(int)(*xinp)));
                    ...
                    *xotp=((float)(j+1) - *yinp)*(((float)(i+1) - *xinp) *
                          NGCALLF(cpmpc2,CPMPC2).xfij[i-1][j-1]+(*xinp-(float)(i)) *
                          NGCALLF(cpmpc2,CPMPC2).xfij[  i][j-1])+(*yinp-(float)(j)) *
                          (((float)(i+1) - *xinp)*NGCALLF(cpmpc2,CPMPC2).xfij[i-1][j]+
                          (*xinp-(float)(i))*NGCALLF(cpmpc2,CPMPC2).xfij[i][j]);
                    *yotp=((float)(j+1) - *yinp)*(((float)(i+1) - *xinp) *
                          NGCALLF(cpmpc2,CPMPC2).yfij[i-1][j-1]+(*xinp-(float)(i)) *
                          NGCALLF(cpmpc2,CPMPC2).yfij[  i][j-1])+(*yinp-(float)(j))*
                          (((float)(i+1) - *xinp)*NGCALLF(cpmpc2,CPMPC2).yfij[i-1][j]+
                          (*xinp-(float)(i))*NGCALLF(cpmpc2,CPMPC2).yfij[i][j]);
                }
                else {
                    *xotp= *xinp;
                    *yotp= *yinp;
