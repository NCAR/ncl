...
int main()
{
	...
	extern void ccpllb(float*);		    /* demo driver */
...
*/	
{
	...
	extern void color(int); /* define color table */

				/* fill area routine */
	extern int fill(float*, float*, int*, int*, int*, int*);


/* Set fill style to solid, and turn off clipping */
      	gset_fill_int_style(1);
	gset_clip_ind(0);

/* Make room at the bottom for the information label */
        c_cpsetr("VPB - VIEWPORT BOTTOM",0.1);

/* Set up label box options */
      	c_cpseti("LLP - LINE LABEL POSITIONING FLAG",2);
      	c_cpseti("LLB - LINE LABEL BOX FLAG",1);
      	c_cpseti("HLB - HIGH/LOW LABEL BOX FLAG",1);
      	c_cpseti("ILB - INFORMATIONAL LABEL BOX FLAG",1);
      	c_cpsetr("HLW - HIGH/LOW LABEL WHITE SPACE WIDTH",0.);
      	c_cpsetr("ILW - INFORMATIONAL LABEL WHITE SPACE WIDTH",.03);
