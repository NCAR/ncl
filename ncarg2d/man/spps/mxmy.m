...
thus, MXMY is considered an obsolete routine.
.sp
...
codes.  If you are writing new code, we suggest that you keep track of
the pen location through internal bookkeeping.
.sp
...
.SH DESCRIPTION 
.IP IX 12
(an output integer) is the X coordinate of the pen position in PAUs.
.IP IY 12
(an output integer) is the Y coordinate of the pen position in PAUs.
.SH USAGE
The routines CURVE, FRSTPT, LINE, PLOTIF, POINT, POINTS, and VECTOR
update the pen position for subsequent MXMY retrieval.  ALso, the
routine PWRIT leaves the pen position at the location specified by
