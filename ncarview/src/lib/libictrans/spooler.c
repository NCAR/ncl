/*
 *	$Id: spooler.c,v 1.2 1991-01-09 11:13:58 clyne Exp $
 */
/***********************************************************************
*                                                                      *
*                          Copyright (C)  1990                         *
	...
	char	*s;
{
	...
	Args	*parse_trans_args();


	/*
	 * make a copy of the input string
	 */
	input_string = icMalloc (strlen (s) + 1);
	...
	if (*s != ':') {	/* trans arg must be terminated by ':'	*/
		if (alias) cfree (alias);
		if (input_string) cfree (input_string);
		return (NULL);
	}
	...
	if (num_filt) {
		filter = (Args *) icMalloc (num_filt * sizeof (Args )); 
	}
	...
	for (count = 0; count < num_filt; count++) {
		...
		if ((filt = parse_filter_args( s )) == NULL) {
			if (alias) cfree (alias);
			if (input_string) cfree (input_string);
			if (trans_args) cfree ((char *) trans_args);
			if (filter) cfree ((char *) filter);
	...
	spooler = (Spooler *) icMalloc (sizeof (Spooler));
	...
	char	*s;
{
	...
	argv = (char **) icMalloc ((argc + 1) * sizeof (char **));
	...
	args = (Args *) icMalloc (sizeof (Args ));
...
InitSpool()
{
	...
	spoolers.spool = (Spooler *) 
		icMalloc (SMALL_MALLOC_BLOCK * sizeof (Spooler));
	spoolers.num = 0;
	 ...
	 */
	sys_spool_file = icMalloc (strlen ((char *) LIBPATH) 
			+ strlen ("/")
			+ strlen ((char *) SPOOL_FILE)
			+ 1);

	(void) strcpy (sys_spool_file, (char *) LIBPATH);
	...
	if ((home = getenv ("HOME")) != NULL) {
		home_spool_file = icMalloc (strlen (home) 
				+ strlen ("/.")
				+ strlen (SPOOL_FILE)
				+ 1);
	...
	if (sys_spool_file) cfree (sys_spool_file);
	if (home_spool_file) cfree (home_spool_file);
}
	...
	char	*alias;
{
	 ...
	 */
	if (spool_list)  cfree ((char *) spool_list);
	spool_list = (char **) icMalloc ((spoolers.num +1) * sizeof (char **));
...

{
	...
	t_argv = (char **) icMalloc ((t_argc +1) * sizeof(char **));
