        ...
        if(0 == strcmp(type_name, "string"))
        {
           ...
           */
            ...
            if(is_vlstr)
            {
                ...
                *(NrmQuark*)attnode->value = NrmStringToQuark(cp);
              /*
               *fprintf(stderr, "\tfile: %s, line: %d\n", __FILE__, __LINE__);
               *fprintf(stderr, "\tvalue: <%s>\n", NrmQuarkToString(*(NrmQuark*)attnode->value));
               */
                free(cp);
            }
            else
        ...
        case H5O_TYPE_GROUP:
            if(it_is_root)
            {
               ...
               */
                ...
                H5Aiterate2(obj_id, H5_INDEX_NAME, H5_ITER_INC, NULL, _checkH5attribute, &grpnode->att_rec);
            }
            else
            {
                curgrpnode = _addGroup(&grpnode, name);
                H5Aiterate2(obj_id, H5_INDEX_NAME, H5_ITER_INC, NULL, _checkH5attribute, &curgrpnode->att_rec);
            }
        ...
        case H5O_TYPE_DATASET:
            {
                ...
                char parent_group_name[MAX_NCL_NAME_LENGTH];
                char data_name[MAX_NCL_NAME_LENGTH];
               ...
               */
                ...
                H5Aiterate2(obj_id, H5_INDEX_NAME, H5_ITER_INC, NULL, _checkH5attribute, &curvarnode->att_rec);
            }
            break;
                               ...
                               _searchH5link_func_t _searchH5link)
{
   ...
   *fprintf(stderr, "\tgrp_name: <%s>, fid=%d\n", grp_name, fid);
   */
...
static herr_t _readH5info(NclFileGrpNode **rootgrp)
{
    ...
    if(H5O_TYPE_GROUP == oi.type)
    {
       ...
       */
        ...
        /* Get ID for group */
        gid = H5Gopen2((*rootgrp)->fid, root_name, H5P_DEFAULT);
        if(gid < 0)
        {
...
static int _buildH5dimlist(NclFileGrpNode **rootgrp)
{
    ...
    NclFileDimNode   *grpdimnode = NULL;
    NclFileDimNode   *vardimnode = NULL;

    int i, j, n;
    int found_new;
    NclQuark old_dim_name;
    NclQuark new_dim_name;
   ...
   */
    ...
    for(i = 0; i < grpnode->att_rec->n_atts; i++)
    {
        ...
        if(attnode->name == theatt)
        {
            ...
            {
                NHLPERROR((NhlFATAL,NhlEUNKNOWN,
                    "NclNewHDF5: Error retrieving global attribute (%s) from (%s)",
...
static void *H5CreateFile(void *rootgrp,NclQuark path)
{
    NclFileGrpNode *grpnode = (NclFileGrpNode *)rootgrp;
    int fid = 0;
    int ret, mode;
   ...
   */
    ...
    if(fid > 0)
    {
        ...
        return(H5OpenFile(rootgrp, path, -1));
    }
    else
    {
        return(NULL);
...
herr_t _writeH5variableAttribute(hid_t did, NclFileAttNode *attnode)
{
    ...
    else
    {
       ...
       */
        ...
        if(attnode->n_elem > 1)
            ret = H5Sset_extent_simple(aid, 1, &(attnode->n_elem), NULL);
        else
            ret = H5Sset_extent_simple(aid, 0, &(attnode->n_elem), NULL);
    }
                                 ...
                                 long *start, long *finish, long *stride)
{
   ...
   */
    ...
    if(NCL_enum & varnode->type)
    {
      ...
      /*
        size *= _NclSizeOf(varnode->type ^ NCL_enum);
       */
        fprintf(stderr, "\tfile: %s, line: %d\n\n", __FILE__, __LINE__);
        fprintf(stderr, "\tsize = %d\n", size);
