      SUBROUTINE GAQD(NLAT,THETA,WTS,WORK,LWORK,IERROR)
...
C     PARTITION WORK SPACE FOR DOUBLE PRECISION EIGENVALUE(VECTOR COMPUTATION)
      ...
      I2 = I1+2*N
      I3 = I2+2*N
      CALL GAQD1(N,THETA,WTS,WORK(I1),WORK(I2),WORK(I3),IERROR)
      IF (IERROR.NE.0) THEN
      IERROR = 3
      RETURN
      END IF
...
C     COMPUTE WEIGHTS AND POINTS ANALYTICALLY WHEN N=2
      ...
      END IF
      END
      SUBROUTINE GAQD1(N,THETA,WTS,W,E,WRK,IER)
...
C     COMPUTE EIGENVALUES  OF MATRIX
      ...
      INDX = 2*N+1
      NP1=N+1
      CALL DRST(N,N,W,E,MATZ,WRK(INDX),IER)
      IF (IER.NE.0) RETURN
