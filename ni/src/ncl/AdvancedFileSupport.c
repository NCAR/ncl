...
void _CopyGrp(NclFileGrpNode *target_grpnode, NclFileGrpNode *source_grpnode)
{
    NclFileGrpRecord *grprec;
    NclFileGrpNode *grpnode;
    int n = -1;

    target_grpnode->gid = source_grpnode->gid;
    target_grpnode->fid = source_grpnode->fid;
    target_grpnode->pid = source_grpnode->pid;
    
    target_grpnode->pname = source_grpnode->pname;
    target_grpnode->path = source_grpnode->path;
    target_grpnode->name = source_grpnode->name;
    target_grpnode->real_name = source_grpnode->real_name;
    target_grpnode->extension = source_grpnode->extension;
    target_grpnode->file_format = source_grpnode->file_format;
    target_grpnode->status = source_grpnode->status;
    target_grpnode->open = source_grpnode->open;
    target_grpnode->format = source_grpnode->format;
    target_grpnode->define_mode = source_grpnode->define_mode;
    target_grpnode->compress_level = source_grpnode->compress_level;
    target_grpnode->is_chunked = source_grpnode->is_chunked;
    target_grpnode->use_cache = source_grpnode->use_cache;
    target_grpnode->cache_size = source_grpnode->cache_size;
    target_grpnode->cache_nelems = source_grpnode->cache_nelems;
    target_grpnode->cache_preemption = source_grpnode->cache_preemption;

    target_grpnode->chunk_dim_rec = source_grpnode->chunk_dim_rec;
    target_grpnode->unlimit_dim_rec = source_grpnode->unlimit_dim_rec;
    target_grpnode->dim_rec = source_grpnode->dim_rec;
    target_grpnode->att_rec = source_grpnode->att_rec;
    target_grpnode->var_rec = source_grpnode->var_rec;
    target_grpnode->coord_var_rec = source_grpnode->coord_var_rec;
    target_grpnode->udt_rec = source_grpnode->udt_rec;
    target_grpnode->parent = NULL;

    _NclCopyGroupOptions(target_grpnode, source_grpnode);

    grprec = source_grpnode->grp_rec;
    ...
    if(NULL != grprec)
    {
        for(n = 0; n < grprec->n_grps; n++)
        {
            ...
            AddNewGrp((void*)target_grpnode, grpnode->name, grpnode->fid);
        }

        for(n = 0; n < grprec->n_grps; n++)
        {
            _CopyGrp(target_grpnode->grp_rec->grp_node[n],
                     source_grpnode->grp_rec->grp_node[n]);
        }
    }
}
