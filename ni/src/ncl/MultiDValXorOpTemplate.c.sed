...
/*
 *      $Id: MultiDValXorOpTemplate.c.sed,v 1.2 1994-12-23 01:18:02 ethan Exp $
 */
/************************************************************************
*									*
*			     Copyright (C)  1994			*
 ...
 *
 *	Date:		Thu Jan 13 14:55:10 MST 1994
 *
 *	Description:	
 */
...
#endif
{
	...
	result_val = (void*)NclMalloc(totalelements * sizeof(OUTDATATYPE));
	if(result_val == NULL) {
...
*/	
	...
	if(!(self_md->multidval.missing_value.has_missing) &&
		...
		for(i=0 ; i< totalelements; i++) {
			res[i] = (OUTDATATYPE)(!ls[i]&&rs[i])||(ls[i]&&!rs[i]);
		}
	} else if(!(other_md->multidval.missing_value.has_missing)) {
		res = (OUTDATATYPE*)result_val;
		ls = (DATATYPE*)self_md->multidval.val;
...
#endif
{
	...
	totalelements = self_md->multidval.totalelements;
	result_val = (void*)NclMalloc(sizeof(OUTDATATYPE)*totalelements);
	if(result_val == NULL) {
...
*/
	...
	if(!(self_md->multidval.missing_value.has_missing ) &&
		!(other_md->multidval.missing_value.has_missing)) {
			...
			rs = (DATATYPE*)other_md->multidval.val;
			for(i=0 ; i< totalelements ; i++) {
				res[i] = (OUTDATATYPE)(!ls[i]&& *rs)||(ls[i]&& !*rs);
			}
	} else if(!(other_md->multidval.missing_value.has_missing )) {
			res = (OUTDATATYPE*)result_val;
			ls = (DATATYPE*)self_md->multidval.val;
...
*/	
	...
	if(!(self_md->multidval.missing_value.has_missing) &&
		!(other_md->multidval.missing_value.has_missing )) {
			...
			rs = (DATATYPE*)other_md->multidval.val;
			for(i=0 ; i< totalelements ; i++) {
				res[i] = (OUTDATATYPE)(!*ls&& rs[i])||(*ls && !rs[i]);
			}
	} else if(!(self_md->multidval.missing_value.has_missing)) {
			res = (OUTDATATYPE*)result_val;
			ls = (DATATYPE*)self_md->multidval.val;
