     ...
     *                  INTYP,ILEV,PSFC,SPVL,KXTRP,
     *                  IMAX,NLAT,NLEVI,NLEVIP1,NLEVO)
C
...
C
     ...
     *          HBCOFA(2,NLEVIP1),HBCOFB(2,NLEVIP1),PLEVI(NLEVI),
...
C
      DO K=1,NLEVI
         ...
         PLEVI(K)  = (HBCOFA(ILEV,KPI)*P0) +
     *               HBCOFB(ILEV,KPI)*(PSFC(I,J)*.01) 
      ENDDO
      IPRINT = 1
      IF (I.EQ.1.AND.J.EQ.1.AND.IPRINT.EQ.1) THEN
         PRINT 601
  601    FORMAT(/,' PRINTOUT FROM VINTHB ')
         PRINT 602,I,J,NLEVI,NLEVO,IMAX,ILEV
  602    FORMAT(' I,J,NLEVI,NLEVO,IMAX,ILEV ',6I4)
         PRINT 603,PSFC(I,J),P0
  603    FORMAT(' PSFC,P0 ',1P2E12.5)
         CALL PRNT('HBCOFA',HBCOFA,2,NLEVIP1,1,1)
         CALL PRNT('HBCOFB',HBCOFB,2,NLEVIP1,1,1)
         CALL PRNT(' PLEVI',PLEVI,NLEVI,1,1,1)
         CALL PRNT(' PLEVO',PLEVO,NLEVO,1,1,1)
      ENDIF
C
...
C
   ...
   25       FORMAT(' KP.GT.KLEVI IN P2HBD.  KP,KLEVI= ',2I5)
            CALL PRNT(' PLEVI',PLEVI,NLEVIP1,1,1,1)
            CALL PRNT(' PLEVO',PLEVO,NLEVO,1,1,1)
...
C
  ...
  102    FORMAT(' DATI(KP),DATI(KP+1),PLEVO(K),',
     ...
     *          /,1X,1P5E12.5)
      ENDIF
         DATO(I,J,K) = DATI(I,J,KP)+(DATI(I,J,KP+1)-DATI(I,J,KP))
     *                 * ALOG(PLEVO(K)/PLEVI(KP))
     *                 /  ALOG(PLEVI(KP+1)/PLEVI(KP))
C
C****     FOR LOG LOG INTERP. NOTE A2LN IS A STATEMENT FCN.
C
      ELSE IF (INTYP.EQ.3) THEN
         DATO(I,J,K) = DATI(I,J,KP)+(DATI(I,J,KP+1)-DATI(I,J,KP))
     2                 * (A2LN(PLEVO(K)) - A2LN(PLEVI(KP)))/
     3                   (A2LN(PLEVI(KP+1)) - A2LN(PLEVI(KP)))
      ENDIF
   40 CONTINUE
C
   50 CONTINUE
