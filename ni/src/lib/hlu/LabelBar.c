...
{NhlNlbBoxLineThicknessF, NhlClbBoxLineThicknessF, NhlTFloat, 
	 ...
	 NhlTString,_NhlUSET("1.0"),0,NULL},
{NhlNlbBoxLineDashPattern, NhlClbBoxLineDashPattern, NhlTDashIndex, 
	 sizeof(NhlDashIndex), 
 ...
 */
	...
	if (lb_p->labels_id < 0) {
		...
		else if (! memcmp(lb_p->label_locs,olb_p->label_locs,
				  sizeof(float) * lb_p->label_draw_count)) {
			NhlSetSArg(&sargs[nargs++],
				   NhlNMtextPosArray,lb_p->label_locs);
		}
