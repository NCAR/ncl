...
static int _PreLayout();
static int _CellAction();
static void DrawIconCell();
static void DrawConnectingLine();
static void BtnPress();
static void Activate();
static void SwitchRowState();
static XmLGridRow _RowNew();
static XmLGridRow _RowNew();
...
static void CreateDefaultPixmaps();
static XmLTreeWidget WidgetToTree();
...
static int _PreLayout(g, isVert)
XmLGridWidget g;
int isVert;
	{
	XmLTreeWidget t;
...
static int _CellAction(cell, w, cbs)
XmLGridCell cell;
Widget w;
XmLGridCallbackStruct *cbs;
	{
...
static void DrawIconCell(cell, w, row, clipRect, ds)
XmLGridCell cell;
Widget w;
int row;
XRectangle *clipRect;
...
static void DrawConnectingLine(dpy, win, gc, clipRect, oddFlag, x1, y1, x2, y2)
Display *dpy;
Window win;
GC gc;
XRectangle *clipRect;
...
static void BtnPress(w, closure, event, ctd)
Widget w;
XtPointer closure;
XEvent *event;
Boolean *ctd;
...
static void Activate(w, clientData, callData)
Widget w;
...
static void SwitchRowState(t, row, event)
XmLTreeWidget t;
int row;
XEvent *event;
	{
...
static XmLGridRow _RowNew(tree)
Widget tree;
	{
	XmLGridWidgetClass sc;
	XmLTreeRow row;
...
/* Only to be called through Grid class */
static void _GetRowValue(g, r, value, mask)
XmLGridWidget g;
XmLGridRow r;
XtArgVal value;
long mask;
...
/* Only to be called through Grid class */
static int _SetRowValues(g, r, mask)
XmLGridWidget g;
XmLGridRow r;
long mask;
	{
...
static void CreateDefaultPixmaps(t)
XmLTreeWidget t;
	{
	Display *dpy;
	Window win;
...
static XmLTreeWidget WidgetToTree(w, funcname)
Widget w;
char *funcname;
	{
	char buf[256];
...
Widget XmLCreateTree(parent, name, arglist, argcount)
Widget parent;
char *name;
ArgList arglist;
Cardinal argcount;
...
void XmLTreeAddRows(w, rows, count, position)
Widget w;
