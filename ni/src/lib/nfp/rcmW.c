...
extern void NGCALLF(drcm2pointstest,DRCM2POINTSTEST)(int *,int *,int *,
						     double *,double *,
						     double *,int *,double *,
						     double *,double *,
						     double*,int *,int *,
...
NhlErrorTypes rcm2points_test_W( void )
{
/*
 * Input variables
 */
 ...
 */
  if(dsizes_lat2d[0] != dsizes_lon2d[0] ||
     dsizes_lat2d[1] != dsizes_lon2d[1]) {
    NhlPError(NhlFATAL,NhlEUNKNOWN,"rcm2points_test: The input lat/lon grids must be the same size");
    return(NhlFATAL);
  }
  ...
  if(dsizes_lon1d[0] != nlat1d) {
    NhlPError(NhlFATAL,NhlEUNKNOWN,"rcm2points_test: The output lat/lon arrays must be the same length");
    return(NhlFATAL);
  }

  if(nlon2d < 2 || nlat2d < 2 || nlat1d < 1) {
    NhlPError(NhlFATAL,NhlEUNKNOWN,"rcm2points_test: The input lat/lon grids must have at least 2 elements, and the output lat/lon arrays 1 element");
    return(NhlFATAL);
  }
 ...
 */
  if(ndims_fi < 2) {
    NhlPError(NhlFATAL,NhlEUNKNOWN,"rcm2points_test: fi must be at least two dimensions");
    return(NhlFATAL);
  }
  if(dsizes_fi[ndims_fi-2] != nlat2d || dsizes_fi[ndims_fi-1] != nlon2d) {
    NhlPError(NhlFATAL,NhlEUNKNOWN,"rcm2points_test: The rightmost dimensions of fi must be nlat2d x nlon2d, where nlat2d and nlon2d are the dimensions of the lat2d/lon2d arrays");
    return(NhlFATAL);
  }
/*
 * Compute the sizes of the input/output arrays.
 ...
 */
  if((nlon2d > INT_MAX) || (nlat2d > INT_MAX) || (ngrid > INT_MAX) || (nlat1d > INT_MAX)) {
    NhlPError(NhlFATAL,NhlEUNKNOWN,"rcm2points_test: one or more input dimension sizes is greater than INT_MAX");
    return(NhlFATAL);
  }
  inlon2d = (int) nlon2d;
  inlat2d = (int) nlat2d;
 ...
 */
  if(type_fi == NCL_double) {
    ...
    missing_fo.floatval = missing_rfi.floatval;
    if(tmp_fo == NULL) {
      NhlPError(NhlFATAL,NhlEUNKNOWN,"rcm2points_test: Unable to allocate memory for temporary array");
      return(NhlFATAL);
    }
  }
  ndims_fo  = ndims_fi-1;
  dsizes_fo = (ng_size_t*)calloc(ndims_fo,sizeof(ng_size_t));
  if(fo == NULL || dsizes_fo == NULL) {
    NhlPError(NhlFATAL,NhlEUNKNOWN,"rcm2points_test: Unable to allocate memory for output array");
    return(NhlFATAL);
  }
  for(i = 0; i < ndims_fi-2; i++) dsizes_fo[i] = dsizes_fi[i];
  dsizes_fo[ndims_fi-2] = nlat1d;
 ...
 */
  ...
  if(tmp_lat2d == NULL || tmp_lon2d == NULL ||
     tmp_lat1d == NULL || tmp_lon1d == NULL || tmp_fi == NULL) {
    NhlPError(NhlFATAL,NhlEUNKNOWN,"rcm2points_test: Unable to coerce input lat/lon arrays to double precision");
    return(NhlFATAL);
  }
 ...
 */
	if (!strcmp(attr_list->attname, "k")) {
	  if(attr_list->attvalue->multidval.data_type != NCL_int) {
	    NhlPError(NhlWARNING,NhlEUNKNOWN,"rcm2points_test: The 'k' attribute must be an integer, defaulting to 2.");
	    k = 2;
	  }
	  else {
	    k = *(int*) attr_list->attvalue->multidval.val;
	    if(k <= 0) {
	      NhlPError(NhlWARNING,NhlEUNKNOWN,"rcm2points_test: The 'k' attribute is < 0. Defaulting to 2.");
	      k = 2;
	    }
	    else {
	      set_k = True;
 ...
 */
  ...
  NGCALLF(drcm2pointstest,DRCM2POINTSTEST)(&ingrid,&inlat2d,&inlon2d,tmp_lat2d,
					   tmp_lon2d,tmp_fi,&inlat1d,tmp_lat1d,
					   tmp_lon1d,tmp_fo,&missing_dfi.doubleval,
					   opt,&tmp_ncrit,&k,&ier);
  if(ier) {
    if(ier == 1) {
      NhlPError(NhlWARNING,NhlEUNKNOWN,"rcm2points_test: not enough points in input/output array");
    }
    if(2 <= ier && ier <= 5) {
      NhlPError(NhlWARNING,NhlEUNKNOWN,"rcm2points_test: lat2d, lon2d, lat1d, lon1d must be monotonically increasing");
    }
    set_subset_output_missing(fo,0,type_fo,size_fo,missing_dfi.doubleval);
  }
  else {
