...
extern void nccf_make_tripolar_hgrid(int nxbnds, int nybnds, double *xbnds, double *ybnds, int *nlon, 
					int *nlat, double lat_join, int *ndivx, int *ndivy,
					char *gridname, char *center, char *history);

NhlErrorTypes nccfmaketripolarhgridW(void)
{
...
int ndims_lat_join;
ng_size_t dsizes_lat_join[NCL_MAX_DIMENSIONS];
NclScalar missing_lat_join;
int has_missing_lat_join;
NclBasicDataTypes type_lat_join;
...
lat_join= (double *)NclGetArgValue(
	6,
	11,
	&ndims_lat_join,
	dsizes_lat_join,
	&missing_lat_join,
	&has_missing_lat_join,
	&type_lat_join,
	2);
...
nccf_make_tripolar_hgrid(*nxbnds, *nybnds, xbnds, ybnds, nlon, nlat, *lat_join,
