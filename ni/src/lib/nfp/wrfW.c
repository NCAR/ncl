...
extern void NGCALLF(virtual_temp,VIRTUAL_TEMP)(double *tmp,double *ratmix,
...
/* This is untested as of March 10 2014. Fortran routine is not compilable yet. */
 ...
 * Test dimension sizes.
 */
  nx = dsizes_prs[ndims_prs-1];
  ny = dsizes_prs[ndims_prs-2];
  nz = dsizes_prs[ndims_prs-3];
 ...
 * Test dimension sizes.
 */
  nx = dsizes_qvp[ndims_qvp-1];
  ny = dsizes_qvp[ndims_qvp-2];
  nz = dsizes_qvp[ndims_qvp-3];
...
/* This is untested as of March 10 2014. Fortran routine is not compilable yet. */
NhlErrorTypes wrf_virtual_temperature_W( void )
{
/*
 * Input array variables
 */
 ...
 */
  if(ndims_temp < 3 || ndims_temp != ndims_ratmx) {
    NhlPError(NhlFATAL,NhlEUNKNOWN,"wrf_virtual_temperature: The temp and ratmx arrays must have at least 3 dimensions and have the same number of dimensions as each other");
    return(NhlFATAL);
  }
  for(i = 0; i < ndims_temp; i++) {
    if(dsizes_temp[i] != dsizes_ratmx[i]) {
      NhlPError(NhlFATAL,NhlEUNKNOWN,"wrf_virtual_temperature: temp and ratmx must be the same dimensionality");
      return(NhlFATAL);
    }
  }
 ...
 */
  ...
  if(nx > INT_MAX) {
    NhlPError(NhlFATAL,NhlEUNKNOWN,"wrf_virtual_temperature: nx = %ld is greater than INT_MAX", nx);
    return(NhlFATAL);
  }
  if(ny > INT_MAX) {
    NhlPError(NhlFATAL,NhlEUNKNOWN,"wrf_virtual_temperature: ny = %ld is greater than INT_MAX", ny);
    return(NhlFATAL);
  }
  if(nz > INT_MAX) {
    NhlPError(NhlFATAL,NhlEUNKNOWN,"wrf_virtual_temperature: nz = %ld is greater than INT_MAX", nz);
    return(NhlFATAL);
  }
  inx = (int) nx;
  iny = (int) ny;
 ...
 */
  ...
  if(type_temp != NCL_double) {
    ...
    if(tmp_temp == NULL) {
      NhlPError(NhlFATAL,NhlEUNKNOWN,"wrf_virtual_temperature: Unable to allocate memory for coercing 'temp' to double");
      return(NhlFATAL);
    }
  }
  else {
  ...
  if(type_ratmx != NCL_double) {
    ...
    if(tmp_ratmx == NULL) {
      NhlPError(NhlFATAL,NhlEUNKNOWN,"wrf_virtual_temperature: Unable to allocate memory for coercing 'ratmx' to double");
      return(NhlFATAL);
    }
  }
  else {
 ...
 */ 
  if(type_tv == NCL_double) {
    ...
    if(tv == NULL) {
      NhlPError(NhlFATAL,NhlEUNKNOWN,"wrf_virtual_temperature: Unable to allocate memory for output array");
      return(NhlFATAL);
    }
  }
  else {
 ...
 * Call Fortran routine.
 */
      NGCALLF(virtual_temperature,VIRTUAL_TEMPERATURE)(tmp_temp,tmp_ratmx,tmp_tv,
                                       &inx,&iny,&inz);
