...
c                                             nw >=3 arbitrary
   ...
   20         CONTINUE
      ...
      END
c ----------------------------------------------------------- 
c This is a test version of rcm2points to deal with a bug
c (NCL-893) with certain curvilinear grids that have narrow
c sections that curve downward on the edges. If testing on
c this goes well, it should replace DRCM2POINTS above.
c
C NCLFORTSTART
      SUBROUTINE DRCM2POINTSTEST(NGRD,NYI,NXI,YI,XI,FI,NXYO,YO,XO,FO
     +                           ,XMSG,OPT,NCRIT,KVAL,IER)
      IMPLICIT NONE
      INTEGER NGRD,NXI,NYI,NXYO,OPT,NCRIT,KVAL,IER
      DOUBLE PRECISION XI(NXI,NYI),YI(NXI,NYI),FI(NXI,NYI,NGRD)
