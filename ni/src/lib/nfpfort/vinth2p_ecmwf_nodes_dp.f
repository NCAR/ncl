C NCLFORTSTART
      SUBROUTINE DVINTH2PECMWFNODES
     +                        (DATI,DATO,HBCOFA,HBCOFB,P0,PLEVI,PLEVO,
     +                         INTYP,ILEV,PSFC,SPVL,KXTRP,NPTS,
...
C
C****     ON INPUT-
C****        DATI    - 3 DIMENSIONAL ARRAY (J,KI) CONTAINING DATA
...
c Variable is "T" and ECMWF extrapolation is desired
     ...
     +                                      2000.D0)*TPLAT)
                              ...
                              IF (TPRIME0.LT.TSTAR) THEN
                                  ALNP = 0.D0
                              ELSE
                                  ALNP = RD* (TPRIME0-TSTAR)/PHIS(J)*
     +                                   LOG(PLEVO(K)/PSFCMB)
                              END IF
                          END IF
                          DATO(J,K) = TSTAR* (1.D0+ALNP+.5D0*ALNP**2+
     +                                  1.D0/6.D0*ALNP**3)
                          GO TO 40
