!vc06.res

*vpXF : 0.1
*vpWidthF : 0.8
*vpYF : 0.95
*vpHeightF : 0.8

*vcLevelColors : (/ 28,30,32,34,36,38,40,42,45,48,51,54,57,60 /)
*vcRefLengthF : 0.04
*vcMinFracLengthF : 0.25
*vcFillArrowWidthF : 0.12
*vcFillArrowMinFracWidthF : 0.3
*vcFillArrowHeadMinFracXF : 0.2
*vcFillArrowHeadMinFracYF : 0.35
*vcFillArrowEdgeWidthF : 3.0
*vcRefAnnoOn : False
*vectorplot*pmLabelBarDisplayMode : conditional
*vectorplot*pmLabelBarWidthF : 0.8
*vectorplot*pmLabelBarHeightF : 0.1
*vectorplot*pmLabelBarSide : bottom
*vectorplot*lbOrientation : horizontal
*vectorplot*lbTitleString : Sea Level Pressure
*vectorplot*lbTitleFontAspectF : 0.9
*vectorplot*lbBackgroundColor : 2
*vectorplot*lbPerimOn : False
*vectorplot*pmLabelBarOrthogonalPosF : 0.05
*vectorplot*tiDeltaF : 1.0

*wkColorMap : temp1
*mpLimitMode : latlon
*mpMinLatF : 0.0
*mpFillOn : True
*mpFillDrawOrder : predraw
*mpOutlineDrawOrder : draw
*mpLabelsOn : False
*mpPerimOn : False
*mpGeophysicalLineColor : 2
*mpGridAndLimbDrawOrder : draw
*mpGridLineColor : 2
*mpLimbLineColor : 2
*mpPerimOn : false
*mpLabelsOn : false
*mpEllipticalBoundary : true
